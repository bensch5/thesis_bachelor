...
unsere vorher aufgestellten Regeln für einen \gls{ac:gbs} umgeht, sondern eine logische Konsequenz aus diesen. In Abbildung TODO REF ist dies noch einmal
zusätzlich verdeutlicht.

% TODO: Abbildung zu Splicing
...
% KURZ nochmal beschreiben, wie beim Aufbauen der Hierarchie nach möglichen Regeln gesucht wird
% erst einen teil matchen, dann den rest rekursiv, ...
Nach der Erzeugung eines neuen Graphen \(L\) in der Hierarchie betrachten wir stets alle vorher erzeugten Graphen \(R\) und vergleichen die Boundaries
...
Betrachten wir erneut das Beispiel in Abbildung \ref{fig:set_of_graphs}. Wir haben einen Graphen \(L\) mit der Boundary \(\partial L = abcd\wedge \).
...
Ein Graph \(R_2\) mit \(\partial R_2 = bc\wedge \) kann ebenfalls gefunden werden und alle Teile von \(\partial L\) konnten anderen \gls{ac:gbs} zugeordnet
werden. Somit kann nun eine neue Produktionsregel erstellt und \(L\) aus der Hierarchie entfernt werden, da sich dieser von simpleren Graphen vereinfachen
lässt.
...
kann es beliebig viele Regeln geben, jedoch zumindest immer eine. Ohne eine Starter-Regel können keine Graphen aus der Grammatik abgeleitet werden,
da der leere Graph keine Kanten besitzt, welche man den Graphen in den anderen Produktionsregeln zuordnen könnte. Eine solche Starter-Regel kann durch
...
Hierarchie entfernt und eine neue Produktionsregel abgleitet, welche auf der linken Seite den vollständigen Graphen \(L\) und auf der rechten Seite den
...
% stubs (5.5)
% unvervollständigbare Graphen (5.6)
